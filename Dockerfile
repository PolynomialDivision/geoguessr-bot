...
RUN --mount=type=cache,id=shared-cargo-git,target=/usr/local/cargo/git \
    ...
    --mount=type=cache,id=geoguessr-bot-target,target=/build/target \
...
RUN --mount=type=cache,id=shared-cargo-git,target=/usr/local/cargo/git \
    ...
    --mount=type=cache,id=geoguessr-bot-target,target=/build/target \
    cargo build --release && \
    cp target/release/geoguessr-bot /geoguessr-bot
...
COPY --from=builder /geoguessr-bot /usr/local/bin/geoguessr-bot

VOLUME /app/store
VOLUME /app/config
WORKDIR /app
CMD ["geoguessr-bot", "/app/config/config.toml"]
